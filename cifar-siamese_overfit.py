...
from keras.layers.convolutional import Conv2D, MaxPooling2D, _Conv
from keras.layers.core import Activation, Dense, Dropout, Flatten, Lambda
from keras.models import Sequential, Model
from keras.layers import Input, merge, LeakyReLU, BatchNormalization, Dense
...
class Attention(Layer):
    ...
    def get_config(self):
        ...
        config['ch'] = 128 # say self. _localization_net  if you store the argument in __init__
...
def create_base_network2(input_shape):
    input = Input(shape=input_shape)
    # x =Flatten()(input), 先加上一个卷积网络再说吧
    x = Conv2D(20, (3, 3), activation='relu', padding='same', input_shape=input_shape, data_format="channels_last")(
        input)
    # Conv2D(20,(3,3), activation='relu', padding ='same', input_shape=)
    x = Flatten()(x)
    # 最后的 dense 中第一个参数在这里可以理解为embedding size
    x = Dense(128, activation='relu')(x)
...
def create_base_network_attention(input_shape, embedding_size =128):
    ...
    seq.add(Dense(embedding_size))
    seq.add(Dropout(0.2))
    # donot use 0.5 , too much for cifar 10
...
def create_base_network_attention(input_shape, embedding_size =128):
    seq =Sequential()
    seq.add(SNConv2D(32,(7,7),strides=(2,2),padding="same",input_shape=input_shape))
    seq.add(BatchNormalization())
    seq.add(LeakyReLU(0.2))


    seq.add(SNConv2D(64,(5,5),strides=(2,2),padding="same"))
    seq.add(BatchNormalization())
    seq.add(LeakyReLU(0.2))

    seq.add(SNConv2D(128,(3,3),strides=(2,2),padding="same",input_shape=input_shape))
    seq.add(BatchNormalization())
    seq.add(LeakyReLU(0.2))

    # add attention layer
    seq.add(Attention(128))

    seq.add(SNConv2D(256,(3,3),strides=(2,2),padding="same"))
    seq.add(BatchNormalization())
    seq.add(LeakyReLU(0.2))
...
# 这个使用了不同的数据集，一个是training dataset
# create training+test positive and negative pairs
digit_indices = [np.where(y_train == i)[0] for i in range(num_classes)]
tr_pairs, tr_y = create_pairs(x_train, digit_indices)
# 数据增强有两个地方，一个是在原始的图像中，一个是在形成pair的时候。衡量一下，还是在后者这里代码量比较小，就是在之后的这里加上三四种不同的
# 图像处理的方式。 on the fly, 最后需要对应好标签。tr_y的增加就是 数量的增加。


digit_indices = [np.where(y_test == i)[0] for i in range(num_classes)]
te_pairs, te_y = create_pairs(x_test, digit_indices)

# network definition 这个是模型的定义
...
# use history to trace the procedure of training
...
plt.tight_layout()
#plt.show()
plt.savefig('loss-accuracy_overfit3-21.png', bbox_inches='tight')

# save and reload model
# so easy hhhhh
model.save('mymodel_overfit3-2.h5')
del model

"""
from keras.models import load_model
model =load_model('mymodel.h5', custom_objects={'contrastive_loss':contrastive_loss})
y_pred2 = model.predict([tr_pairs[:, 0], tr_pairs[:, 1]])
print('the shape of y_pred2:',str(y_pred2.shape))
"""
...
n_iters =1
result_path ="result_overfit3-2.txt"
...
model =load_model('mymodel_overfit3-2.h5', custom_objects={'contrastive_loss':contrastive_loss,
                                                          'SNConv2D':SNConv2D,
                                                          'Attention':Attention})
